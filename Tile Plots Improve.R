...
TILE.MAX.SUM <- Summer_Max %>% 
  ...
  scale_x_discrete(expand = c(0,0),
                   breaks = c("1910","1920","1930","1940","1950",
                              "1960","1970","1980","1990","2000","2010")) +
  scale_fill_manual(values=StatePalette,
                    na.value = "#fcf5e1")+ 
  #coord_fixed()+
  theme_grey(base_size = 10)+
  theme(legend.position = "right",
...
TILE.MIN.SEASONS <- All_Seasons_1960 %>% 
  # order by station, & average max temperature 
  mutate(name = fct_reorder(Station, -Min_Average)) %>%
  ggplot(aes(x = Year,
             y = name,
  ...
  labs(x = "",
       y = "",
       title = "Average yearly minimum temperature in Australia across the seasons (°C)",
       subtitle = "Temperature recordings from 112 weather stations (y-axis) between 1960 & 2019 (x-axis)",
       caption = "Dataset: The Australian Climate Observations Reference Network – Surface Air Temperature (ACORN-SAT)"
  ) +
  scale_y_discrete(expand = c(0,0)) +
  ...
  theme(legend.position = "right",
        legend.direction = "vertical",
        legend.title = element_text(colour = textcold,
                                    size=10),
        legend.margin = margin(grid::unit(0,"cm")),
        legend.text = element_text(colour = textcold,
                                   size = 8,
                                   face = "bold"),
        legend.key.height = grid::unit(0.8,"cm"),
        legend.key.width = grid::unit(0.2,"cm"),
        ...
        legend.background = element_rect(fill = "#f0fcfc"),
        panel.border = element_blank(),
        strip.text.x = element_text(size = 9, colour = textcold),
        strip.background = element_rect(fill="#f0fcfc"),
        plot.margin = margin(0.7,0.4,0.1,0.2,"cm"),
        plot.title = element_text(colour = textcold,
                                  hjust = 0,
                                  size = 11,
...
ggsave(TILE.MIN.SEASONS,
       filename="All-Seasons-min.png",
       height = 8.8,
       width = 16.8,
       units = "in",
       dpi = 200)
